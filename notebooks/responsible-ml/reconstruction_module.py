...
def load_data():
    ...
    data = data.drop(columns=['APPLICANT_ID', 'CITY', 'COUNTRY', 'AGE_GROUP',
       'APPLICATION_DATE', 'APPLICATION_TYPE', 'DEGREE_TYPE',
       'CREDIT_CARD', 'SAVINGS_ACCOUNT', 'ONLINE_REQUEST', 'EMPLOYMENT_LENGTH', 'APPROVED_BACKGROUND_CHECK',
       'CHILDREN_COUNT', 'OWN_REALTY', 'OWN_CAR', 'SHOULD_APPROVE'])
    data['agebinned'] = pd.cut(data['AGE'], bins = range(20, 51, 5), right = False) 
    data['EMPLOYED'] = data['EMPLOYED'].astype('category')
...
def reconstruct_data(model, elem_dict):
    ...
    df = pd.DataFrame(columns = ['DEGREE', 'EMPLOYED', 'agebinned', 'SEX', 'MARRIED', 'INCOME'])
    for elem in full_elem_list:
        ...
        df['DEGREE'] = df['DEGREE'].astype('int').astype('category')
        df['EMPLOYED'] = df['EMPLOYED'].astype('int').astype('category')
